...
また、codex プロセス自体が起動しない場合（Claude Code 側の permission 拒否、`codex` バイナリ未インストール、認証エラーで即時 exit 等）も **失敗扱い** とする。この場合は codex 出力が無いため、原因として Claude Code / 環境側のエラーメッセージを引用する。

**注意:** codex は正常時も `sandbox: <mode>` をヘッダに出力するため、裸の `sandbox` 文字列を判定に使ってはいけない。上記の具体パターンのみを使うこと。

上記に該当しなければ成功。**判定は重複除去前の原文に対して行う**（重複除去は報告フォーマット適用時のみ）。
...
- codex が `Review comment:` 形式の単一段落、または `codex\n` ラベル直後に裸の段落で返すことがある。3 節構造（指摘事項 / 推奨事項 / 評価）に分かれていない場合は一律 **codex 出力の本文を「指摘事項」に丸ごと原文のまま貼る**（勝手に日本語訳・要約・配分しない）。
...
- codex が **同一文または同一段落** を 2 回以上連続で繰り返した場合（codex 側の既知の挙動）は 1 回にまとめてよい（重複除去のみ許可、内容は一字一句変えない）。文単位の重複と段落単位の重複の両方が起きた場合は段落単位を優先（より大きい単位で集約）。
